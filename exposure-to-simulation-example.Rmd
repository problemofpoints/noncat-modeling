...
```{r export-sims}
export_large_losses <- large_losses %>% 
  mutate(sequence = sequence + 1,
         event_id = trial * 1e6 + sequence) %>% 
  select('trialid' = trial, event_id, loss)

export_attr_losses <- total_losses %>% 
  mutate(sequence = 1,
         event_id = trial * 1e6 + sequence) %>% 
  select('trialid' = trial, event_id, 'loss' = loss_attr)

readr::write_csv(export_large_losses %>% select(-trialid), "one_lob_example_large.csv")
readr::write_csv(export_attr_losses %>% select(-trialid), "one_lob_example_attritional.csv")
